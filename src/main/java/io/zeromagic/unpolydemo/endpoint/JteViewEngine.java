...
public class JteViewEngine extends ViewEngineBase {
    ...
    public void processView(ViewEngineContext viewEngineContext) throws ViewEngineException {
        ...
        var finalView = view;
        try (var writer = new PrintWriter(viewEngineContext.getOutputStream(), false, resolveCharsetAndSetContentType(viewEngineContext))) {
            if (raw) {
                render(viewEngineContext, new PrintWriterOutput(writer), view, null);
            } else {
                Content content = (output) -> render(viewEngineContext, output, finalView, null);
    ...
    private void render(ViewEngineContext viewEngineContext, TemplateOutput output, String finalView, Content content) {
